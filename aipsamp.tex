...
\author{Maya Segal}
\affiliation{ 
Dept. Chemistry and Biochemistry, University of California Los Angeles, CA, USA
}%
...
532nm and 628nm (2RU-VFL-Series, MPB Communications Inc., QC, Canada).
...
FL, USA) used for \textgreek{μ}s time-scale modulation, while the 532nm laser is
unmodulated. Each laser goes through a first beam expander (Keplerian
...
X10468-07, Hamamatzu, Japan), before passign through the dichroic mirror
...
(D) and acceptor (A) spectral bands. The D path goes thorugh a band-pass
filter (Brightline FF01-582/75, Semrock) which removes residual 628nm
laser leakage and helps suppressing any Raman scattering from the 532nm
laser. Both D and A paths are refocused by lenses \(L5_D\) /
\(L5_A\) (f=150mm, AC254-150-A, Thorlabs) on two 48-pixels
SPAD arrays\cite{Gulinatti_2013} (denoted as donor and acceptor SPAD in the
following).

Both SPAD arrays are place on 3-axis micropositioners. The directions
...
coaligned so that, for each spot, there is a maximum overlap between D
...
To this end, without the recollimating len \(L3\), an iris
...
the objective and generates a spot from the coverglass reflection. Only
...
high concentration solution of dyes (100nM - 1\selectlanguage{greek}μ\selectlanguage{english}M) used for alignment.
...
excited by the 628nm laser. Using the camera we align the red pattern to
...
12x4 pattern (\(\pm\)1mm) on the LCOS-SLM.
...
ATTO647N at high concentration (\(\sim\)100nM). Emission due
to 532nm and 628nm laser is acquired with two subsequent acquisition and
...
contour of the Gaussian waist for 532nm (\emph{green}) and 628nm
...
ATTO647N, \textasciitilde{}500nM) that is excited by both lasers. With
this sample the 532nm laser yields fluorescence signal in both D and A
channel, while the 628nm laser only yields signal in the A channel. The
center of the excitation pattern for both 532nm and 628nm laser is
...
\begin{figure*}
\includegraphics[width=0.70\textwidth]{{figures/2017-05-23_08_12d_phrates_summary}}
...
\begin{figure*}
\includegraphics[width=0.70\textwidth]{{{figures/2017-05-23_08_12d_48spot_alex_hist_Su_all-bursts}}}
...
\begin{figure*}
\includegraphics[width=0.70\textwidth]{{figures/2017-05-23_08_12d_48spot_alex_hist_Su_naa_AND_size_selection}}
...
\begin{figure}
\includegraphics[width=0.8\columnwidth]{{figures/2017-05-23_08_12d_FRET_vs_DO_fitted_E-Su_peak_position}}
...
\begin{figure*}
\includegraphics[width=0.70\columnwidth]{{figures/2017-05-05_05_12d_48-spot_E-Su_peak_position_deviation_from_mean}}
...
alignment. The 4x4 LCOS pattern is excited with the 532nm laser and
...
green LCOS under 532nm illumination and moving the A-SPAD with picomotor
...
the red LCOS under 628nm excitation, and the center location of the red
...
(\usalex is one of the most powerful methods.
...
smFRET \emph{periodic acceptor excitation} (smFRET-PAX)
...
\(A_{ex}\) (respectively D or A excitation) and two (D and A)
detectors. This results in four basic photon streams named
\(F_{DexDem}, F_{DexAem}, F_{AexDem}, F_{AexAem}\). The \(F_{AexDem}\) stream only contains
...
Similarly to ALEX, in PAX we have two (D and A) detectors and two laser
alternation periods for D and D+A excitations, named \(D_{ex}\)
and \({DA}_{ex}\) respectively. As in ALEX, combining the two
excitation periods and the two detectors, we obtain four basic PAX
photon streams named \(F_{DexDem}, F_{DexAem}, F_{DAexDem}, F_{DAexAem}\). Formally, the only difference
...
particular, \(F_{DAexDem}\) contains D fluorescence due to D laser
excitation, while the corresponding term \(F_{AexDem}\) in ALEX
