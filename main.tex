...
intensities available in {\usalex} (see section~\ref{sec:EShist}).
$E$ is either the FRET efficiency or, more commonly
...
Fig.~\ref{fig:phrates48_comp} shows the background-corrected peak photon
rate distribution in the 48 spots. Panel A shows the full peak photon rate
...
% * <michalet@chem.ucla.edu> 2017-07-29T00:06:03.848Z:
%
% > shows
% A reporter " reports on". A figure does not. At most, it " reports"  something.
%
% ^.
...
range from Gaussian fits of $E_{PR}$ and $S$ histograms
...
range, with the exception of spot 19, where as note before
a lower PDE of the A pixel causes a larger deviation from the peak position.
...
It is possible to further reduce spot-to-spot variations
...
(22 bp D-A separation) is reported in appendix~\ref{sec:moredata},
Fig.~\ref{fig:alexhist48all22d}.
...
The final step of multispot measurements merging data from all
spots in order to accumulate bursts at an higher pace. In principle,
...
we do not any spot-specific adjustment.
...
resulting mean $E_{PR}$ of $\sim0.6$ and $0.15$ respectively.
A significant D-only population is visible as peak close to $E_{PR}=0.05$
...
Fig.~\ref{fig:ES48mix-fret} shows the same E-S histogram after
D-only filtering. For this purpose,
we set a threshold of 12.5 photons on the background-corrected
$F_{DA_{ex}A_{em}}$ counts in each burst.
We notice that the selection effectively removes the large D-only
peak, isolating the FRET populations without a significant loss
of FRET bursts.

In order to showcase the high-throughput capability of the multispot
system we compared short acquisitions of the same sample in both the
multispot and a standard single-spot {\usalex} setup.
In Fig.~\ref{fig:alex_mspot_5}, we report the E-S histogram
built from a 5~s acquisition of a doubly-labeled dsDNA measured
on the 48-spot PAX setup.
For comparison, Fig.~\ref{fig:alex_sspot_5} shows a measurement of the
same sample obtained accumulating bursts for 5~s on the single-spot
{\usalex} setup.
To minimize differences in concentration,
the same gasket-enclosed sample was first measured
on the {\usalex} setup and subsequently measured in the multispot setup.
In both cases we performed the same burst search with a constant
rate-threshold of 50~kcps.
For burst selection, we used a minimum threshold of 40 photons
for the $gamma$-corrected burst size.
In principle, we expect a 46X "gain" in burst accumulation rate
in the multispot setup (48 minus two non-functional spots). However,
many factors contribute reducing this theoretical limit.
First, the decreasing power in the lateral spots (see
Fig.~\ref{fig:patternfit}) causes smaller bursts and
results in less bursts surviving the size-selection with a given threshold.
In addition, the detection efficiency of the SPAD array in the red region
of the spectrum is at least 2-fold lower than that of the corresponding
SPAD in the single-spot setup (see section~\ref{sec:detectors}).
This more than halves the signal in the A-SPAD, resulting in smaller bursts
and, as before, less bursts above a given threshold.
Both issues can be in principle solved in future iterations of the setup.
The excitation uniformity can be increased
using flat-top optics before the LCSO-SLM modulation,
while the lower PDE can be overcome using new red-enhanced SPAD
arrays~\cite{gulinatti_new_2012}.

Overall we note that a significant number of bursts can be obtained
in just a few seconds of acquisition. In realtime kinetic experiments
a single multipsot acquisition is analyzed in time slices, following
the evolution of the system over time. The time-resolution
in these experiments depends on the information that needs to be extracted
from each time slice.
...
In this case, 500 bursts per slice maybe sufficient,
resulting in a time resolution < 5~s with the current setup.
To avoid dead time during mixing, however, the multispot setup needs to be
equipped with a microfluidic mixer than can mix and flow a sample
in $\ll$~1~s.

\begin{figure}
\includegraphics[width=\columnwidth]{figures/2017-07-16_02_12d_1000pM_alex_jointplot_5s_Dex}
...
\begin{figure}
\includegraphics[width=\columnwidth]{figures/2017-07-16_003_12d_alex_jointplot_5s}
