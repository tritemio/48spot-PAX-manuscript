...
The performances of the systems are demonstrated on a set of doubly-labeled
...
to realtime kinetics studies of enzymatic reactions such as DNA transcription
...
macromolecular complexes is important for understanding of their
...
fluorescently-active A dye do not emit fluorescence when the A-laser is on
...
The setup includes two CW excitation laser (green: 532~nm, red: 628~nm)
...
photon timestamping (12.5~ns resolution) and data transfer to an host PC.
The host PC runs an acquisition software that displays 96-channel timetraces
in realtime, implements alignment routines and saves the acquisition to
...
Full details regarding these samples are provide in
...
a two-dimensional histograms where each bursts is represented by a
...
Due to the choice of donor and acceptor excitation powers during this measurement, the FRET population has an $S_u$ value larger than 0.5, artificially compressing the histograms in the upper part of the graphs. Nonetheless, it is still possible to distinguish FRET from D-only bursts, despite the low  value of $E_{PR}$ for that sample, allowing a bias-free estimation of $E_{PR}$, by contrast of what would have happened in the absence of  acceptor excitation [cite 8-spot paper].
...
burst search are identical as in figure~\ref{fig:alexhist48all12d}, while
burst selection has been tailored to select only the FRET population:
% * <michalet@chem.ucla.edu> 2017-06-22T12:29:56.838Z:
% 
% > only the FRET population
% that doesn't sound right, since the DO population is visible
% 
% ^.
a burst is selected if the number of 
counts in the $D_{ex}DA_{em}$ stream is larger than 20 and counts larger than 20 in the
$DA_{ex}A_{em}$ stream. The legend in each subplot
reports spot number ($[\cdot]$) and number of bursts (\#B).%
}}
\end{figure*}
