...
\noindent where $W_A$ and $W_D$ are the durations of the $D_{ex}$ and
$DA_{ex}$ excitation periods.
...
we start defining the "modified" total FRET signal as:
...
is divided by $\alpha$, which is the $D_{ex}$ duty-cycle
($\alpha = 0.5$ when alternation periods have same duration).
The factor $\alpha^{-1}$ amplifies the A signal in order to compensate
for the additional donor signal $F_{DA_{ex}D_{em}}$.
