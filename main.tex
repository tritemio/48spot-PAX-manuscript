...
Single-molecule Förster resonance energy transfer (smFRET) has paved the way
...
setups, but with a throughput that scaled with the number
of excitation spots.
...
Briefly, in {\usalex},
two exciting lasers are alternately turned on and off every few tens of
...
active (FRET population). In fact, only FRET populations
give a fluorescence signal both during the D-laser excitation due to
...
the same time, in order to maximize the signal, the detection PSF must be
...
Once a reasonable (coarse) alignment is obtained, we use a more automated procedure for fine alignment, consisting briefly in moving the LCOS pattern along a path forming a small cross (typically less than a pattern unit cell length),  in order to look for the (X, Y) offset  resulting in the largest intensity. We refer to this procedure as a
``multispot scan''. Specificaly, a multispot scan involves
translating a small subset of the full spot pattern (typically 4x4 spots) on one of the LCOS-SLM in
small steps along two orthogonal directions, forming a cross path.
Counts from the corresponding SPADs in either
array are accumulated for a fixed duration (typically 300~ms) at each pattern position to form a intensity profile for each of these SPADs along each scan direction.
A typical scan covers a range of 10 LCOS-SLM pixels in each direction with a step size of 0.4 pixels (there are ~20 pixels per spot pattern).
For each SPAD, the intensity profile along each cross' arm can be fitted by a Gaussian function, providing an estimated peak position along both directions. This offset indicates the optimal LCOS-SLM spot location for each SPAD. Because we want to move the SPAD array, we use the averaged offset for all SPADs in the small 4x4 pattern as an
estimation for the SPAD array offset.  We then move the SPAD array to this new improved position using software-controlled piezo micro-positioners.
The sequence of multispot scan and SPAD array translation is repeated a few times
until convergence.
This procedure is repeated separately for both LCOS-SLM patterns. First, the two SPAD arrays are aligned with respect to the green (532 nm)
LCOS-SLM pattern. Next, the position of the red (628 nm) LCOS-SLM pattern
is adjusted to match the position of the A-SPAD array (the 
...
patterns, and (c) to optimize the SPAD position along the optical axis
(Z).
...
the scan fits. Once the orientations of two SPAD arrays match, the rotation
stage is locked, ensuring long-term stability of the rotational angle.

Regarding the pitch, information from the scan fits is used to
...
due to non-idealities due to stigmatism in the optical path.
...
and A-SPADs center positions are denoted with '+'. 
}
\end{figure}
...
define two periods, one when only the D-laser is on ($D_{ex}$) and
...
background-to-noise ratios are significantly higher.
...
was not recovering absolute FRET values and D-A distances, but rather to
demonstrate the capabilities of the smFRET-PAX system in terms of
spot uniformity and throughput gain. Therefore, we did not compute a
complete gamma factor calibration. However, we addressed the differences
in collection and detection efficiency across different spots using
