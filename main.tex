...
The main result of the {\usalex} analysis is the E-S histogram,
a 2-D histograms where each bursts is represented by a
pair of values $(E, S)$. $E$ is the FRET efficiency or, more commonly,
the proximity ratio $E_{PR}$.  The latter being is an easy to compute
approximation that is suitable for the common case of identify sub-populations.
Conversely, when the purpose is extracting D-A distances,
$E$ needs to be computed, requiring and accurate estimation of
all the correction coefficients.
$S$ is the "stoichiometry", a quantity ideally centered around 0.5 for
dual-labeled species, while being close to 0 for A-only and close to 1
for D-only species. When using the simpler form without correction
(eq.~\ref{eq:S}), $S$ can have a dependency on $E$ and, for dual-labeled
molecules and is only roughly centered around 0.5.
The use of the $E$ and $S$ pair (corrected or uncorrected)
allow separating singly- and doubly-labeled species and, among the
doubly-labeled ones, distinguishing FRET populations.

In this paper we report proximity ratios $E_{PR}$ computed according to
eq.~\ref{eq:Epr}, and a ``modified stoichiometry'' $S_u$ defined
in eq.~\ref{eq:Su}. $S_u$ is a variant of the classical
smFRET-PAX\cite{Doose_2007} stoichiometry which reduces
...
Moreover, the results of two spots (the two leftmost in the second row) are missing,
...
\subsection{Pooling all channels}
To illustrate the high-throughput performances of the 48-spot system we
build the ALEX histogram from a single pool of bursts from all the channels
taking only a slice of 15~s of measurement (Fig.~\ref{fig:alex_mspot_15}).
As a comparison, Fig.~\ref{alex_sspot_15} we report the analogous results
obtained in 15~s of measurement of the same sample with the standard
single-spot {\usalex} setup.
...
sample as in~\ref{fig:alex_mspot_15}.}
\end{figure}
...
Conflict of interest statements: S. Weiss discloses equity in
...
\subsection{Signal optimization for coarse alignment of LCOSs to SPADs}
...
\subsection{Coalignment of SPADs to LCOS pattern by scanning}
...
\subsection{Fine X-Y alignment of SPADs}
...
\subsection{X-Y pitch and rotation of green and red LCOSs by scan fits}
...
$F_{FRET}$ is the A fluorescence due to FRET, computed
...
We also define the correction factors $\gamma$ and $\beta$:
...
\noindent where $\phi_A$, $\phi_D$ are dyes quantum yield
and $\eta_{A_{det}}^{A_{em}}$, $\eta_{D_{det}}^{D_{em}}$ are PDEs of
D and A channels.
In eq.~\ref{eq:beta}, $I_{A_{ex}}$ and $I_{D_{ex}}$ are A and D excitation
intensities, while $\sigma_{A_{ex}}^A$ and $\sigma_{D_{ex}}^D$
are the dyes absorption cross-section at the wavelength of the respective laser.
$\beta$ accounts for the difference of D and A dye fluorescence when
each dye is excited by its respective lasers.
