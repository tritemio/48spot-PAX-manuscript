...
The final step of multispot measurements is merging data from all
spots in order to accumulate bursts at an higher rate. In principle,
...
system, in Fig.~\ref{fig:alex_mspot_5} and~\ref{fig:alex_sspot_5}
we show an example of a short 5~s acquisitions of the same doubly-labeled
dsDNA sample in the multispot and a standard single-spot {\usalex} setup
respectively.
The sample has a D-A separation of 12 bp, and was described
...
rate-threshold of 50~kcps and similar burst-size selection.
At first glance, we would expect a theoretical 46X "gain" in burst
accumulation rate in the multispot setup (48 minus two non-functional spots).
However, many factors may affect the burst rate.
...
This reduces the signal in the A-SPAD, resulting in smaller bursts
and, less bursts above a given threshold.
Moreover, The two setups use different set of emission path filters which
can further affect the detected photon rate, burst size and $\gamma$ factor.
Finally, analysis of the burst duration histogram show that the spots are
generally larger in the multi-spot setup, due to the specific modulation
which does not generate diffraction-limited PSFs. Larger PSF volumes
cause an higher number of bursts in the multi-spot compared to the
single-spot setup.

The issues of uniformity and low A-channel PDE can be solved
in future iterations of the setup.
