...
laser excitation,
the technique allows resolving the number of fluorescent
labels on each molecule, which greatly enhances the ability to study
...
Here we demonstrate a high-throughput single-molecule FRET system which
multiplexes the acquisition by using 48 excitation spots and
48-pixels SPAD array detectors. The system employs two excitation lasers
allowing the separation of species with one or two active fluorophores.
The performances of the system are demonstrated on a set of doubly-labeled
double-stranded DNA oligos
...
to real-time kinetics studies of enzymatic reactions such as DNA transcription
...
macromolecular complexes is important to understand their
...
for attaining atomic-detailed macromolecular structures. More recently,
single-particle cryo-electron microscopy (cryo-EM) has complemented these
methods for the determination of large macromolecular structures with the
...
It allows the determination of each conformational state that may exist in
the ensemble of macromolecular complexes as well as the distance between
specific residues for each state.
Recently, several groups used smFRET to
measure distances between multiple distinct pairs of residues for
constructing 3D macromolecular structures of distinct conformations by
...
Moreover, smFRET can, in principle, measure evolution in time of multiple
distances between multiple pairs, hence report of the dynamic 3D structure
of a macromolecule undergoing conformational changes,
but due to the requirement of low sample concentration, imposed by the
necessity to have no more than 1 molecule within the diffraction-limited
confocal volume at a given time~\cite{deniz_single-pair_1999},
only very slow kinetics could be measured.
Therefore, both static and dynamic measurements of multiple
distances could greatly benefit from increased throughput.
...
number of SPAD pixels in each detection channel).
We illustrated an application of this enhanced throughput
by measuring, in bacterial transcription,
the bubble closing kinetic upon promoter escape~\cite{ingargiola_multispot_2017}.
...
or bleached. Since the latter categories are present in most samples,
it is important to be able to identify and separate them from the
...
doubly-labeled species, due to the fact that molecules lacking an
fluorescently-active A dye does not emit fluorescence when the A-laser is on
(and the donor laser is off).
...
identify in each single molecule the number of D and A dyes (stoichiometry)
...
% * <michalet@chem.ucla.edu> 2017-06-21T16:47:03.885Z:
%
...
with only the one (red) being modulated via an acoustic-optic modulator (AOM).
...
The two lasers are then combined by a dichroic mirror ($DM_{MIX}$),
recollimated ($L3$) before being focused into the sample by an
high numerical aperture (NA) objective lens (60x, NA=1.2).
The emitted fluorescence is collected by the same objective lens,
separated by the excitation through a dual-band polychroic mirror ($DM_{EX}$),
and focused by the tube lens ($L2$) into an image plane.
Next, the fluorescence light is recollimated ($L4$) separated in donor
and acceptor spectral bands ($DM_{EM}$) and focused in two different
48-pixel SPAD arrays mounted on motorized micro-positioning stages.
The system is aligned so that each SPAD pixel is optically
conjugated to one excitation spots into the sample.

The detectors output is processed by an FPGA-based board which performs
photon timestamping (12.5~ns resolution) and data transfer to a host PC.
...
To easily integrate the detectors into a multispot setup,
 ...
 array and the electronics required for device operation, data acquisition and transmission into
 a compact, user friendly module.
In particular, the SPAD array is housed into a hermetically 
sealed chamber separated from the rest of the module. Sealing in a dry atmosphere makes
 it possible to mount the array on a double-stage Peltier and to cool
the detector down to temperatures of about -15 $^{\circ}$C, thereby reducing
the dark count rate (DCR) and increasing the sensitivity of the instrument.
Photon counting pulses are made externally available through a standard SCSI connector. This allows for an easy connection of the module to general-purpose boards for data acquisition and processing. 
Alternatively, an onboard FPGA (Xilinx Spartan 6, model SLX150)
can be used to synchronize the 48 channels, time-stamp each detected
count with a time resolution of 10 ns and send this information
to a remote PC  through a high-speed USB link.
A C-mount at the top of the photon counting module 
allows also for an easy and reliable connection to the multispot setup.

The two SPAD arrays used in the current 48-spot smFRET-PAX setup are operated at a temperature of -10 $^{\circ}$C.
The photon detection efficiency (PDE) reaches a maximum of $\sim$45\%
at around 550~nm, dropping to about 35\% at
628nm\cite{gulinatti_48-pixel_2013,michalet_silicon_2014}. The PDE is highly uniform over the array, with a peak-to-peak spread of only a few percent.
Fig.~\ref{fig:dcr} shows the heatmaps of dark count rates of the two 12x4 SPAD arrays. About 80\% of the pixels have a DCR lower than 1000 counts per second (cps), whereas the worst pixel has a fairly low DCR of less than 6 kcps.
...
characterized by similar PDE as our SPAD arrays in the D spectral band
but more than two-fold higher PDE in the A spectral band.
...
\begin{figure}
    ...
    \caption{{\label{fig:dcr} Heatmaps of Dark Count Rates (DCR) of the two 12x4
...
The 48 excitation spots are generated independently for each wavelength
via phase modulation of an incoming plane wave, as
previously described
in~\onlinecite{colyer_high-throughput_2010,ingargiola_multispot_2017}.
The phase modulation operates in the directs space (as opposed to Fourier space)
and implements the phase profile a Fresnel lenslet array.

Fig.~\ref{fig:patternfit} shows the emission pattern due to green (Panel A)
and red laser (Panel B) acquired by the camera
on the microscope side-port using a high-concentration dye sample. The two
patterns are aligned to maximize the overlap of each of the 48 spots.
The overlap of the two wavelengths and centering with respect to
...
\includegraphics[width=\columnwidth]{{figures/2017-04-28_conf9_G_conf14_R_green_red_pattern_and_fit}}
    ...
    Fluorescent images obtained upon 532~nm or 628~nm laser excitation were acquired separately and are reported in green and red intensity levels in panels (A) and (B), respectively.
    ...
    (C) reports an overlay of the fitted peak positions peak positions and
...
attached to different DNA bases and yielding different inter-dye distances.

D-A separation of 7, 12 and 22 base pairs (7d, 12d, 22d) were used in these
experiments,
...
$E$ needs to be computed, requiring accurate estimation of
...
for D-only species. Note that D-only and A-only species also include
doubly-labeled molecules where one dye is fluorescently inactive
(blinking off state, bleached).
...
Panels B-E show, for the different photon streams,
...
it is clear that the lower sensitivity in the A-SPAD array results in lower peak
...
histograms for the different spots in the 12d dsDNA sample. Two
populations are visible: D-only (around $E_{PR}=0$,
...
histograms for the different spots in the 12d dsDNA sample. Data  analysis and
burst search are identical to figure~\ref{fig:alexhist48all12d}, while
burst selection was tailored to select only the FRET population: a burst is selected if the number of
counts in the $D_{ex}DA_{em}$ and $DA_{ex}A_{em}$ streams are both larger than 20. The legend in each subplot
...
Fig.~\ref{fig:alexhist48all12d} shows the E-S histogram for the 12d sample,
...
on E-S peak position by applying the per-channel calibration reported in~\ref{sec:perchcorr}.
...
arrays~\cite{colyer_ultra_2011,burri_65k_2014,buchholz_fpga_2012,kufcsak_time-resolved_2017,kufcsak_time-resolved_2017},
...
The authors thank Luca Miari for helping testing the SPAD arrays,
Mr. Yazan Alhadid for help with single-molecule
sample preparation and Dr. Eitan Lerner for help with single-molecule
sample preparation and for critical reading of the manuscript.
We thank Dr. Bentolila for the generous loan of a LCOS SLM by the
...
Each SPAD arrays is equipped with a USB 2.0 connection for low-resolution
...
focuses an array of spots in a LCOS image plane at a distance of 3-4 cm
from the modulator surface (see Fig.~\ref{fig:setup}).
A rectangular region of the LCOS-SLM is subdivided into 12x4 adjacent blocks
...
resulting in an LCOS-SLM lenslet pitch of 463~{\micron} (23.1 LCOS-SLM pixels).
...
Using a square grid of spots with an odd number of spots per side
...
channel, while the 628~nm laser only generates a signal in the A channel.
...
translating a multispot pattern (typically 4x4 spots) on an LCOS-SLM in
...
$D_{ex}D_{em}$, $D_{ex}A_{em}$, $A_{ex}D_{em}$, $A_{ex}A_{em}$, where the first
symbol indicates the excitation period and the second, the detection channel.
...
\label{modified-stoichiometry}

By replacing $\tilde{F}_{A_{ex}A_{em}}$ with $F_{A_{ex}DA_{em}}$ in eq.
\ref{eq:Spax}, we define the "modified stoichiometry" $S_u$
...
As a result, the
$S_u$ distributions are tighter, permiting easier
...
Fig.~\ref{fig:alexhist48all22d} shows $E_{PR}$-$S_u$ histograms for the different
channels obtained during the measurement of a 22d DNA sample (low-FRET).
Due to the choice of donor and acceptor excitation powers during this measurement, the FRET population has an $S_u$ value larger than 0.5, artificially compressing the histograms in the upper part of the graphs. Nonetheless, it is still possible to distinguish FRET from D-only bursts, despite the low value of $E_{PR}$ for that sample, allowing a bias-free estimation of $E_{PR}$, as opposed to what would have happened in the absence of acceptor excitation~\cite{ingargiola_multispot_2017}.
