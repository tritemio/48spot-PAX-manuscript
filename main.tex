...
characterized by similar PDE in the D spectral band and two-fold higher PDE
...
labeled with ATTO550 (D) and ATTO647N (A) (ATTOTEC, City, State) located at different distances.
% * <michalet@chem.ucla.edu> 2017-06-15T00:24:29.353Z:
% 
% > City, State
% missing info
% 
% ^.
D-A separation of 7d, 12 and 22 base pairs (7d, 12d, 22d) were used in these experiments, as they cover the typical range of distances of interest in single-molecule FRET.
...
the uncorrected FRET efficiency known as proximity ratio, $E_{PR}$.  $E_{PR}$ is easier to compute
and provides an approximation suitable for identifying sub-populations.
However, when the purpose is to extract D-A distances (which is not the objective of this study),
...
smFRET-PAX\cite{Doose_2007} stoichiometry, which reduces the effect of
...
Note that, in all this work, the results of two spots (the two leftmost spots in the second row) are missing,
...
build an ALEX histogram with bursts from all channels within a 15~s window of one measurement (Fig.~\ref{fig:alex_mspot_15}).
For comparison, Fig.~\ref{alex_sspot_15} shows a similar histogram obtained from a 15~s window during a measurement of the same sample using the single-spot {\usalex} setup. The total number of bursts  detected in the multispot measurement is about 12 times larger than in the single spot measurement. This is less than the theoretical factor of 48, which would be expected if all spots in the multispot setup were equivalent to the single-spot setup, but is consistent with the facts that (i) the excitation power in some of the spots is lower than in others, and (ii) the detection efficiency of the SPAD array in the red part of the spectrum is lower than that of the corresponding SPAD in the single-spot setup, reducing total burst size.
% * <michalet@chem.ucla.edu> 2017-06-14T22:12:55.029Z:
% 
% Check that last argument, as I don't know your burst search and selection criteria in the two measurements.
% 
% ^.
% * <michalet@chem.ucla.edu> 2017-06-14T20:47:55.026Z:
% 
% This figure is apparently not found. I suppose this is Fig. 11?
% 
% ^.
...
In summary, we have described a new 48-spot single-molecule FRET setup designed for high-throughput single-molecule assays. Its design incorporate a number of technical solutions of potential interest for other applications, as well as proof of principle demonstration of its single-molecule FRET measurement capabilities. We provide a careful comparison of its performance with that of a standard single-spot (confocal) single-molecule FRET setup. Applications of this new instrument to the study of the initial stages of bacterial transcription and high-throughput diagnostics will be described in future publications.

\begin{acknowledgments}
This work was supported by NIH grants R01 GM095904 \& R01 GM069709 and
NSF grant MCB 1244175.

Conflict of interest statements: S. Weiss discloses intellectual property used in the research
...
We also need the usual correction factors $\gamma$ and $\beta$:
% * <michalet@chem.ucla.edu> 2017-06-15T00:05:01.220Z:
% 
% cite Lee et al, BJ 2005
% 
% ^.
...
% * <michalet@chem.ucla.edu> 2017-06-15T00:05:44.455Z:
% 
% > PDE
% defined previously?
% 
