...
Fig.~\ref{fig:alexhist48all12d} reports on the E-S histograms for the
...
In Fig.~\ref{fig:alex_mspot_15}, we reported raw $E_{PR}$ and $S_u$
values without any spot-specific adjustments for simplicity and because
of the good uniformity across spots.

For comparison, Fig.~\ref{fig:alex_sspot_15} shows a similar histogram
obtained by accumulating bursts for 15~s during a measurement of the
same doubly-labeled dsDNA construct using the
single-spot {\usalex} setup. To minimize differences
in concentration, the same gasket-enclosed sample was first measured
on the {\usalex} setup and subsequently measured in the multispot setup.
The resulting number of bursts detected in the
multispot measurement are about 24 times larger than in the
single-spot measurement. This factor is smaller that the
the 46-fold increase theoretically expected (discarding the two spots
with malfunctioning pixels). This discrepancy can be explained by two factors.
First, the decreasing power in the lateral spots causes smaller bursts and
...
and, as before, less bursts with sizes above a given threshold.
Both issues can be solved in future iterations of the setup.
...
subtracting D-leakage ($Lk$) and A-direct-excitation ($Dir$)
from $F_{D_{ex}A_{em}}$~\cite{lee_accurate_2005}:
...
Definitions of $Lk$ and $Dir$ are reported in
ref.~\onlinecite{lee_accurate_2005}.
...
Note that if the alternation periods have different duration
(i.e. duty cycle $\ne 0.5$),
eq.~\ref{eq:pax_Faa} should be modified by introducing weights in the
subtraction that account for the actual duty cycle.
...
Since the D-excitation is always on in PAX experiments, we can use
the signal in $F_{DA_{ex}D_{em}}$ to improve photon statistics, and
derive a modified set of PAX
...
%
% ^.
expressions for $E$ and $S$, starting from
...
In the above expressions, the acceptor contribution $F_{FRET}$,
is multiplied by a factor 2, to compensate for the addition of
$F_{DA_{ex}D_{em}}$.
In case of non-equal alternation periods, this factor must be modified
to take into account the actual duty cycle.
...
shot-noise is amplified, an effect that can become large for low FRET
