...
(10~mM Tris pH 8.0, 1 mM EDTA, and 50~mM NaCl) or in ``transcription'' buffer (described in ref.\cite{lerner_PNAS_2016}).
...
The three steps of analysis include:
...
The main result of the {\usalex} and PAX analysis methods is a so-called E-S histogram,
...
requiring accurate estimation of all the correction coefficients.
$S$, or "stoichiometry ratio", is a quantity ideally centered about 0.5 for
...
of that pixel that is possibly due to a lower applied overvoltage.
...
and center respectively in the 2D histogram, are clearly distinguishable in all spots.
Moreover, as shown in Fig.~\ref{fig:alexhist48fret12d}, 
the FRET population(s) are easily isolated by applying a second burst
selection which sets a threshold on the $DA_{ex}A_{em}$ counts.
...
Burst were selected by total size with the criterion $\Lambda_{\gamma,PAX} > 80$,
using $\gamma = 0.5$ (see eq.~\ref{eq:burstsize_paxe}).
The E-S histogram was built by pooling data from all the spots without 
spot-specific adjustments.
For more details see the notebook
\href{http://nbviewer.jupyter.org/github/tritemio/48-spot-smFRET-PAX-analysis/blob/master/smFRET-PAX\_single\_pop-2017-07-11\_12d-22d-mixture.ipynb}{smFRET-PAX\_single\_pop-2017-07-11\_12d-22d-mixture}.
}
\end{figure}
...
In this work, for simplicity and due to the good uniformity,
we do not apply any spot-specific adjustments.
...
reduces the signal in the A-SPAD, again resulting in smaller and 
fewer bursts above the set threshold.
Third, the two setups use a different set of emission path filters which
can further affect the detected photon rate, $\gamma$ factor, and burst size.
In the analysis, a $\gamma$-corrected burst size
(eq.~\ref{eq:burstsize}) was used to counteract this effect.
Finally, we observe experimentally that burst durations are slightly larger on average in
the multispot setup, indicating larger PSF volumes.
At a given concentration, a larger PSF
will contain an greater number of molecules, increasing the
...
and the lower PDE can be overcome by implementing new red-enhanced SPAD
arrays~\cite{gulinatti_new_2012}.

Fig.~\ref{fig:alex_mspot_5} shows that a significant number
of bursts can be obtained in only a few seconds of acquisition.
Such throughput is advantageous for real-time kinetic experiments,
where a reaction is triggered at a specific time point while the sample
is continuously measured.
In order to follow the evolution of the system over time,
these measurements are analyzed by dividing the acquisition in small time slices.
The time resolution, i.e. the smallest usable time slice,
depends on the burst rate and on what information one aims to extract
from each slice.
...
While mixing can be performed manually with 15-20~s of dead
time~\cite{ingargiola_multispot_2017},
only an automated microfluidic system will allow full exploitation of 
the throughput capabilities of the multispot setup (mixing
dead time $\ll$~1~s).
Such a system could also perform series of kinetic experiments
...
For more details see the notebook
\href{http://nbviewer.jupyter.org/github/tritemio/48-spot-smFRET-PAX-analysis/blob/master/smFRET-PAX\_ALEX-PAX\_comparison\_2017-08-03\_03\_12d.ipynb}{smFRET-PAX\_ALEX-PAX\_comparison\_2017-08-03\_03\_12d}.
}
\end{figure}
