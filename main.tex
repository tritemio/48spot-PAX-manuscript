...
The current 48-spot setup employs two 12x4-pixel SPAD arrays whose
architecture and performance has been presented in~\onlinecite{Gulinatti_2013}.
The active area of each pixel is 50~{\micron} with a 500~{\micron} pitch.
The photon detection efficiency (PDE) has a maximum of $\sim$45\%
at around 550~nm\cite{Gulinatti_2013}.

\begin{figure}
\includegraphics[width=0.4\textwidth]{figures/DCR-comp}
\caption{{\label{fig:dcr} Maps of Dark Counting Rates (DCR) of the two 12x4
SPAD arrays used in the current 48-spot smFRET-PAX setup.}}
\end{figure}

48-spot smFRET-PAX results were compared to state-of-the-art single-spot
{\usalex} setup.
This setup employs two single-pixel MPD SPADs.
...
in this work. The A detector (50~{\micron} diameter, PDM-RE Series,
Micro Photon Devices) employs a different "red-enhanced"
process~\cite{gulinatti_planar_2011,panzeri_silicon_2011,panzeri_single-molecule_2013}.


A detailed description if this
setup is reported in~\onlinecite{Ingargiola_2017}.
we compare 48
. The single-spot setup employs a standard single-pixel SPAD
as D detector (50~{\micron} diameter, PDM Series, Micro Photon Devices
Bolzano, Italy) realized in a similar technology as the SPAD arrays used in this work
...
in eq.~\ref{eq:Su}. The latter is a variant of the classical
stoichiometry used in smFRET-PAX\cite{Doose_2007} that reduces shot noise
...
\caption{{\label{fig:pharates48_comp}
...
\begin{figure}
\includegraphics[width=0.4\columnwidth]{{figures/2017-06-02_002_12d_usALEX_peak_phrate}}
\caption{{\label{fig:pharates_usalex}
...
In the following we show the results single-molecule measurements of
a 12d dsDNA sample performed with both the 48-spot smFRET-PAX and
a {\usalex} setup. The single-spot setup employs a standard single-pixel SPAD
as D detector (50~{\micron} diameter, PDM Series, Micro Photon Devices
Bolzano, Italy) realized in a similar technology as the SPAD arrays used in this work

SPAD (Microphon) (setup in ref.~\onlinecite{Ingargiola_2017}).
We start by analysis the peak photon rate reached in each burst,
a quantity scaling with the peak PSF intensity\cite{Ingargiola_2017}.
Fig.~\ref{fig:pharates48_comp} shows the background-corrected peak photon-rate
in each burst for the 48 spots. Panel A shows the full peak photon rate
distributions with its characteristic exponential tail.
Panel B-E show heatmaps of the peak photon-rate mean value
(i.e. the decay constant of the exponential tail)
for the different photon streams.
In addition, fig.~\ref{fig:pharates_usalex} shows the distribution
of peak photon rates as obtained with the {\usalex} setup.
Due to the 2x lower sensitivity
...
\begin{figure}
\includegraphics[width=0.4\columnwidth]{figures/2017-05-23_08_12d_FRET_vs_DO_fitted_Epr-Su_peak_position}
