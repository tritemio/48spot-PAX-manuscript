...
$E_{PR}$ and $S$ remains quite uniform across the spots
...
After the burst search and selection described in section~\ref{sec:analysis},
Fig.~\ref{fig:alexhist48all12d} reports the E-S histogram for the
...
It is possible, however, to reduce spot-to-spot variations
...
single-spot measurement. The throughput increase is smaller that the
the theoretical 46-fold (discarding the two spots with malfunctioning
pixels). This discrepancy can be explained by two factors.
First, the decreasing power in the lateral spots causes smaller bursts and
results in less bursts after size selection with a given threshold.
...
This halves the signal in the A-SPAD, resulting in smaller bursts
and less bursts selected above a given size threshold.
