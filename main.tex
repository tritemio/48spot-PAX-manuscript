...
description of samples (Section~\ref{sec:samples}) and data analysis (Section~\ref{sec:analysis}).
...
available on GitHub~\cite{multispot_software}.
Software used for data analysis can be found in the 48-spot-smFRET-PAX-analysis
...
\begin{figure}
    ...
    More details and data can be found in the {accompanying DCR analysis notebook}~\cite{nb_dcr}.
}
\end{figure}
...
\includegraphics[width=\columnwidth]{figures/2017-04-28_conf9_G_conf14_R_green_red_pattern_and_fit}
    ...
    More details can be found in the accompanying alignment notebook~\cite{nb_align_folder}.
    }}
\end{figure}
...
ATTO647N (A) dyes (ATTO-TEC GmbH, Siegen, Germany)
...
TE50 buffer was used for all measurements excluding data collected for
Fig.~\ref{fig:alex_mspot_5} and~\ref{fig:alex_sspot_5}.
The transcription buffer reduces photo-bleaching, and it was necessary
so that a single sample could be measured consecutively on both setups
without significant loss of fluorescence.
...
additionally filter bursts with $DA_{ex}A_{em}$ counts larger
...
\begin{figure*}
    ...
    purple: $DA_{ex}A_{em}$.
    See appendix~\ref{sec:alex_pax} for ALEX and PAX streams definitions.
    For more details see the accompanying 48-spot PAX analysis notebook~\cite{nb_12d}.
    }
\end{figure*}

\begin{figure}
    \includegraphics[width=0.7\columnwidth]{figures/2017-06-11_002_12d_usALEX_peak_phrate}
    ...
    80~\textgreek{μ}W.
    Colors correspond to different photon streams.
    Green: $D_{ex}D_{em}$, red: $D_{ex}A_{em}$, purple: $A_{ex}A_{em}$.
    See appendix~\ref{sec:alex_pax} for ALEX and PAX streams definitions.
    For more details see the accompanying single-spot ALEX analysis notebook~\cite{nb_12d_alex}.
    }
\end{figure}
...
\begin{figure}
    ...
    22~bp D-A separation (sample details in Section~\ref{sec:samples}).
    ...
    For more details, see the accompanying notebook for the 48-spot PAX analysis of the 12 and 22 bp mixture~\cite{nb_12d_22d}.
}
\end{figure}
...
22~bp (see Section~\ref{sec:samples}),
...
In multispot measurements, it is in principle possible that a molecule 
detected in one spot could be  detected in a different spot, potentially 
affecting the conclusions drawn from the cumulative data from all spots. Two 
possible scenarios need to be considered: (i) a single-molecule signal detected 
in one spot is detected in another due to crosstalk effects, and (ii) a 
single-molecule detected in one spot diffuses away and is later detected in 
another spot.
The first scenario can be excluded due to the geometry of the setup which ensures 
that there is no overlap between nearby detection volumes and due to the low 
optical crosstalk coefficients of the type of SPAD array detectors used in this 
work~\cite{ingargiola_multispot_2017}.
The second scenario would not affect burst analysis as these events would only
result in a correlation between bursts in different spots over a timescale 
determined by diffusion from one spot to the other
($\Delta t \gg d^2/4D$, with $d \approx 5$~\microsec, 
$D \approx 100\;$\microsec$^2 s^{-1}$, yields $\Delta t \gg 60$ ms for 
two nearest-neighbor spots). 
In practice, we were unable to detect any such cross-correlation in the
measurements reported here, suggesting that, due to the size of and the 
separation between excitation spots, the probability of occurrence of 
...
that a significant number of bursts ($\sim$1,000) can be obtained in only a few seconds
of acquisition.
...
\begin{figure}
    ...
    For more details see the accompanying notebook for comparison of a single-spot to 48-spots~\cite{nb_mspot_sspot_comp}.
    }
\end{figure}
...
(IX-71, Olympus Corp., Waltham, MA) stands, its bottom port sitting
...
available in the picomotor repository in~\onlinecite{multispot_software}.
...
code available in the MultichannelTimestamper repository)
...
available in the transfer\_convert repository in ref.~\onlinecite{multispot_software}.
...
available in the lcos\_multispot\_pattern repository in ref.~\onlinecite{multispot_software}.
...
LCOS pattern fitting notebook~\cite{nb_align_folder}).
...
(for implementation details see the LCOS pattern fitting notebook~\cite{nb_align_folder}).
...
For details see the accompanying SPAD alignment notebook~\cite{nb_align_folder}.
}
\end{figure}
...
\begin{figure*}
    ...
    reports spot number in brackets and number of bursts (\#B).
    For more details see the accompanying 48-spot PAX analysis notebook~\cite{nb_22d}.
    }
\end{figure*}
