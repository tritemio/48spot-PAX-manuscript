...
of the FRET donor-acceptor pair [ref].
...
molecular sorting applications [refs].
...
is left on at all time, while the acceptor laser is alternated.
...
% * <michalet@chem.ucla.edu> 2017-06-17T21:49:11.568Z:
% 
% add a scale bar
% 
% ^.
...
Eq.~\ref{eq:chich} follows from the relation between $E$
and $E_{PR}$ [ref]:
% * <michalet@chem.ucla.edu> 2017-06-19T13:39:51.068Z:
% 
% > [ref]
% add ref Lee 2005
% 
...
From eq.~\ref{eq:EfuncEpr}, we obtain:
...
Formally, we can write $\gamma = \gamma_1 \gamma_2 $, where $\gamma_1$ is associated with a partially corrected
% * <michalet@chem.ucla.edu> 2017-06-19T13:50:25.586Z:
% 
% > Formally
% I haven't verified the following derivation
% 
% ^.
proximity ratio $E_1$ by:

\begin{equation}
E_1 = \frac{1}{1 + \gamma_1 \left( \frac{1}{E_{PR}} - 1 \right)}
\end{equation}

$E$ is a function of the partially
corrected $E_{1}$ obtained by a simple substitution of
...
Eq.~\ref{eq:EfuncE1} expresses the relation between partially corrected $E$, where the full gamma correction is decomposed into the product of partial corrections. In the
multispot case, we apply this property to decompose the gamma correction into
a spot-specific correction and an average correction as in
