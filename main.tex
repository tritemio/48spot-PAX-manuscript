...
via phase modulation of an incoming plane wave direct-space modulation as
previously described in~\onlinecite{colyer_high-throughput_2010,ingargiola_multispot_2017}.
Briefly, the LCOS-SLM
implements the phase profile of a lenslet array that focuses an array of spots
in an image plane at a distance of 3-4 cm from the modulator surface
(see Fig.~\ref{fig:setup}).
For both excitation wavelengths, the pitch and therefore the diameter
of the lenslets is is dictated by the detector geometry and by the
magnification of the optical train.
A change in focal length of the LCOS lenslets, with constant diameter,
results in a change in their NA and therefore spot size.
The chosen LCOS-SLM focal length differs in the green and red pattern
(32~mm for the red and 36~mm for the green)
in order to reduce the difference in PSF sizes between green and
red excitation.
