...
many factors affect the ratio of burst rates between multispot and
...
Finally, experimentally, burst durations are on average larger in
...
bursts per unit time. With all these caveat, we experimentally found
a burst rate gain between 20-50.
% TODO add experimental value and range
...
where a reaction is triggered at a specific time point and the sample
is continuously measured during the kinetic phase.
Realtime kinetic measurements are analyzed in time slices,
following the evolution of the system over time.
The time-resolution such measurements depends on the burst rate and on
what information should to be extracted from each time slice.
...
the throughput capabilities of the multispot setup (mixing in $\ll$~1~s).
