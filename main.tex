...
are phase modulated by two LCOS-SLM generating
48-spots pattern on a plane after each LCOS-SLM (\emph{LCOS image plane}).
...
separated by the excitation via dual-band polychroic mirror ($DM_{EX}$).
and focused by the tube lens ($L2$) into an image plane.
Next, the fluorescence light is recollimated ($L4$) separated in donor
and acceptor spectral band ($DM_{EM}$) and focused in two different
48-pixel SPAD arrays mounted on motorized micro-positioners.
The system is aligned so that each pixel is optically
conjugated to one excitation spots into the sample.

The detectors output is feed to an FPGA-based board which performs
photon timestamping (12.5~ns resolution) and data transfer
to an host PC.
...
TODO: Add brief description of the 48-spot pattern generation that justifies
and introduces Fig.~\ref{fig:patternfit}.
